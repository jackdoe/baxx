...
cat main.go | pv -L 10K -p | curl -XPOST --data-binary @- localhost:8080/v1/upload/$CLIENT/$TOKEN/todzzsz | jq
...
echo 125 | curl --data-binary @- localhost:8080/v1/upload/$CLIENT/$TOKEN/todzzsz | jq
...
echo 123 | curl --data-binary @- localhost:8080/v1/upload/$CLIENT/$TOKEN/todzzsz | jq
